...
- **Discord Bot (`comfy_bot.py`):** Built with `pycord`, handles slash commands (including `/q`, `/handlers`, and `/wake`), message events, and user interactions.
...
- **Wake-on-LAN:** Integrated `/wake` command that uses `wakeonlan` to send magic packets and asynchronously polls the host status via ICMP ping.
...
5.  **Monitoring:** The client monitors execution progress via a persistent WebSocket connection.
6.  **Output:** A background task (`publish_images`) identifies completed prompts and sends the resulting images and a generation summary back to the Discord channel.
