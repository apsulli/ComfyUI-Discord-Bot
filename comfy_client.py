...
class ComfyClient(object):
    ...
    def _connect_websocket(self):
        while True:
            try:
                self._logger.info(
                    f"Attempting to connect to ComfyUI WebSocket at {self._comfy_url}..."
                )
                self._websocket = websocket.WebSocket()
                self._websocket.connect(
                    "{}://{}/ws?clientId={}".format(
                        self._socket_protocol, self._comfy_url, self._client_id
                    )
                )
                self._logger.info("Successfully connected to ComfyUI WebSocket.")
            except Exception as e:
                self._logger.error(
                    f"Failed to connect to ComfyUI WebSocket: {e}. Retrying in 10s..."
                )
                import time
    ...
    def get_image(self, filename, subfolder, folder_type):
        ...
        with urllib.request.urlopen(
            "{}://{}/view?{}".format(self._protocol, self._comfy_url, url_values)
        ) as response:
            return response.read()

    def get_history(self, prompt_id):
        with urllib.request.urlopen(
            "{}://{}/history/{}".format(self._protocol, self._comfy_url, prompt_id)
        ) as response:
            return json.loads(response.read())

    def get_checkpoints(self):
        with urllib.request.urlopen(
            "{}://{}/object_info/CheckpointLoaderSimple".format(
                self._protocol, self._comfy_url
            )
        ) as response:
            return json.loads(response.read())["CheckpointLoaderSimple"]["input"][
                "required"
            ]["ckpt_name"][0]

    def get_system_stats(self):
        with urllib.request.urlopen(
            "{}://{}/system_stats".format(self._protocol, self._comfy_url)
        ) as response:
            return json.loads(response.read())

    def get_prompt(self):
        with urllib.request.urlopen(
            "{}://{}/prompt".format(self._protocol, self._comfy_url)
        ) as response:
            return json.loads(response.read())

    def get_queue(self):
        with urllib.request.urlopen(
            "{}://{}/queue".format(self._protocol, self._comfy_url)
        ) as response:
            return json.loads(response.read())

    def get_history(self):
        with urllib.request.urlopen(
            "{}://{}/history".format(self._protocol, self._comfy_url)
        ) as response:
            return json.loads(response.read())
    ...
    def get_embeddings(self):
        with urllib.request.urlopen(
            "{}://{}/embeddings".format(self._protocol, self._comfy_url)
        ) as response:
            return json.loads(response.read())
